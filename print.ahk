...
/*	for AutoHotkey v2 by oif2003
	Usage:
		...
		p(arr)    => prints 1D or 2D array; if dim > 2, calls po(arr)
						as 	[Item1,Item2,Item3 ....]
						or 	[1] [Item1,Item2,Item3 ....]
							[2] [Item1,Item2,Item3 ....]
								(...so on and so forth...)
		p(obj, 1) => prints object as [key] = value recursively
...
pn()
pn("Print as Object:")
p(a, 1)		;equivalent to pa(a) in this case since Dim(a) > 2
*/
...
pn(x:="") => p(x "`n")
p(x, opt := 0) {
	static SciTE4AHK := WinExist("SciTE4AutoHotkey")
	
	;Use SciTE4AutoHotkey output pane if possible
	isObject(x) ? (opt ? po(x) : pa(x)) : (SciTE4AHK ? fileappend(x, "*") : _p(x)) 
